(function() {
    ...
    /* compositing */
    /* attribute /* double */
	const K_ATTR_GLOBALALPHA = "globalAlpha"; /* (default 1.0) */
	
	CanvasRenderingContext2D.prototype.__defineGetter__(K_ATTR_GLOBALALPHA, function() {
		this.prepare();
		return this.getGlobalAlpha();
	});
	
	CanvasRenderingContext2D.prototype.__defineSetter__(K_ATTR_GLOBALALPHA, function(v) {
    ...
    CanvasRenderingContext2D.prototype.__defineSetter__(K_ATTR_STROKESTYLE, function(v) {
        ...
        if (typeof v === K_TYPE_STRING)
        {
            ...
            this._context.setStrokeSourceRgba(color.r, color.g, color.b, color.a);
        }
    ...
    CanvasRenderingContext2D.prototype.__defineSetter__(K_ATTR_FILLSTYLE, function(v) {
        ...
        if (typeof v === K_TYPE_STRING)
        {
            ...
            v = v.toLowerCase();
            var color = this.parseColorString(v);
            this._context.setFillSourceRgba(color.r, color.g, color.b, color.a);
        }
        else if (v instanceof CanvasPattern)
        {
            this._context.setSource(v.raw);
    ...
    CanvasGradient.prototype.addColorStop = function(offset, colorStr)
    {
        if (this.raw)
        {
			if (typeof offset !== K_TYPE_NUMBER || offset < 0 || offset > 1)
    ...
    CanvasRenderingContext2D.prototype._fontMetrics = null;
    ...
    CanvasRenderingContext2D.prototype.__defineSetter__(K_ATTR_FONT, function(v) {
        ...
        this._font = v;
        this._fontMetrics = this._context.fontMetrics();
    });
    ...
    CanvasRenderingContext2D.prototype.fillText = function(/* DOMString */ text, /* double */ x, /* double */ y, /* optional double */ maxWidth) {
        ...
        if (tm.width > 0)
        {
            ...
            var te = tm.raw;
            var anchor = this.calcAnchor(x, y, te, this._fontMetrics);
            
            // log("text anchor: " + anchor.x + " , " + anchor.y);
			var mode = K_TXT_DRAW_MODE_FILL;
			if (max_w > 0) K_TXT_DRAW_MODE_FILL_CLIP;
			this._context.showText(text, anchor.x, anchor.y, mode, 0);
        }
    };
    ...
    CanvasRenderingContext2D.prototype.strokeText = function(/* DOMString */ text, /* double */ x, /* double */ y, /* optional double */ maxWidth) {
        ...
        if (tm.width > 0)
        {
			...
			var mode = K_TXT_DRAW_MODE_STROKE;
			if (max_w > 0) mode = K_TXT_DRAW_MODE_STROKE_CLIP;
            this._context.showText(text, anchor.x, anchor.y, mode, 0);
        }
    };
    ...
    CanvasRenderingContext2D.prototype.measureText = function(/* DOMString */ text) {
        if (typeof text === K_TYPE_STRING)
        {
            ...
            {
                this.prepare();
                var te = this._context.measureText(text);
    ...
    CanvasRenderingContext2D.prototype.drawImage = function() {
        if (arguments.length === 3)
        {
            ...
            this.prepare();
			this._context.drawImage(image, dx, dy, image.width, image.height);
        }
